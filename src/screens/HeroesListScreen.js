    ...
    return (
        <View style={styles.container}>
            <ImageBackground source={backgroundImage} style={styles.image} resizeMode='stretch'>
                {state.isLoaded ? <TouchableOpacity onPress={() => navigation.navigate('HeroDetails', { item: state.data.data.results[7] })}>
                    <FeaturedHero item={state.data.data.results[7]} />
                ...
                {state.isLoaded ?
                    <View>
                        <FlatList style={styles.list}
                            ...
                            renderItem={({ item }) => {
                                return <TouchableOpacity onPress={() => navigation.navigate('HeroDetails', { item: item })}><HeroItem item={item} /></TouchableOpacity>
                            }} />
